...
platform = 'matlab';
projectName = 'eurotrap-pt';                                               %%%% used in import_trap
...
%simulationDirectory = ...
%     '/media/Data/quantumlab/trap-simulations/G_trap_translume_v2/'; 	   %%%% used in import_trap
simulationDirectory = '/home/nikos/qlab/trap-simulations/eurotrap/';       %%%% used in import_trap. 
                                                                           % This is where the simulation files are stored.
DataNames= 'eurotrap-pt';                                                  %%%% used in import_trap	
...
timestarted = datestr(now,'mm-dd-yyyy-HHhMM');                             %%%% used in import_trap
%file_name = sprintf('%s%c%s',projectName,'_',timestarted);                %%%% used in import_trap
                                                                           % This is the _time_ in '/simulationDirectory/projectName_time_post-processed
matDataPath = ...
  [simulationDirectory,projectName,'_',timestarted,'_','post-processed/']; %%%% used in import_trap
                                                                           % This is where the post processed trap files are stored.
NUM_AXIS  = 21;                                                            %%%% used in import_trap 
                                                                           % Number of trap points per axis used in import_trap
NUM_ELECTRODES  = 22;                                                      %%%% used in import_trap, get_trapping_field, expand_field, dc_potential, post_process_trap
                                                                           % Number of non-ground electrodes, including all DCs, center, RF
%EC01 NUM_USED_ELECTRODES = 22;                                                  %%%% used in import_trap
%EC01                                                                           % If the RF is not biased, set this to NUM_ELECTRODES-1, etc.

electrodeMapping = ...                                                     %%%% used in import_trap, expand_field
            [1 1; 2 1; 3 2; 4 2; 5 3; ...                                  % electrodeMapping determines which voltage goes to which electrodes. It has (NUM_ELECTRODES,2) elements
		    6 3; 7 4; 8 4; 9 5; 10 5; ...                                  % Here define the electrode combinations (for example electrode pairing)
		    11 6; 12 6; 13 7; 14 7; 15 8; ...                              % the convention is physical electrode -> functional electrode
		    16 8; 17 9; 18 9; 19 10; 20 10; ...                            % The last entry (NUM_ELECTRODES) is the RF electrode
		    21 11; 22 11];                                                 % Entry NUM_ELECTRODES-1 (i.e. before the RF) is the center electrode
                                                                           % Warning: this will only handle pairing up of adjacent electrodes
                                                                           %
manualElectrodes = ...                                                     % used in import_trap, expand_field, dc_potential
                   [1 1 0 0 0 ...                                          % manualElectrodes determines the electrodes which are under manual voltage control. It has NUM_ELECTRODES elements
                    ...
                    1 1]';                                                 % If an electrode is grounded, set it under manual control, and do not set a manual voltage for it in set_voltages 
...
usedMultipoles = ...                                                       %%%% used in trap_knobs
               [1 1 1 1 1 0 0 1];                                          % Select which multipoles you want to control
...
axesPermutation = [2 3 1];                                                 %%%% used in import_trap 
...
nStart = 1;                                                                %%%% used in import_trap
nMatTot = 6;                                                               %%%% used in import_trap, get_trapping_field
...
scale = 1/1000;                                                            %%%% used by import_trap, get_trapping_field, post_process_trap, multipole_set_dc, spher_harm_exp
                                                                           % UNITS: if drawing (autocad) dimensions are in microns and output in mm then
                                                                           % scale = 1/1000 if drawing is in mm, scale = 1;
zMin = -630*scale;                                                         %%%% used in import_trap, get_trapping_field
zMax = -510*scale;                                                         %%%% used in import_trap, get_trapping_field
zStep = 20*scale;                                                          %%%% used in import_trap, get_trapping_field
...
regenerate_trap = true;                                                    %%%% used in expand_field
...
%EC01 trap.Configuration.NUM_USED_ELECTRODES = NUM_USED_ELECTRODES;          % Number of electrodes to which DC voltage is applied. If two electrodes are shorted together, they count as one. 
...
trap.Configuration.regeneratedPotentials = regenerate_trap;            % this is true if expand_field is recalculating the potential trap from the spherical harmonic expansion coefficients
