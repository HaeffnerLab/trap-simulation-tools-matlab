function [Is Js Ks]=findsaddle(A,X,Y,Z,dim,Z0,varargin)
% [Is Js Ks]=findsaddle(A,grid,dim,Z0,varargin)
...
% * a string indicating where findsaddle was called from
...
if (dim==3)
	if (numel(size(A)) ~= 3), 
		fprintf('%s: Problem with findsaddle.m dimensionalities.\n',called_from_string); 
		return; 
	end;
	...
	if debug,
        ...
        titl = sprintf('findsaddle called from %s.\nDisable by setting debug argument to false',called_from_string); 
        title(titl);
        hold off; pause; 
	end
	
	if (Is==1)||(Is==size(A,1)), 
		fprintf('%s/findsaddle: Saddle out of bounds in  x (i) direction.\n',called_from_string); 
	end
	if (Js==1)||(Js==size(A,2)), 
		fprintf('%s/findsaddle: Saddle out of bounds in  y (j) direction.\n',called_from_string); 
	end
	if (Ks==1)||(Ks==size(A,3)), 
		fprintf('%s/findsaddle: Saddle out of bounds in  z (k) direction.\n',called_from_string); 
	end
end
...
if (dim==2)
	...
	if m>1e-3
		if debug,
		Is = NaN; Js = NaN;
		fprintf('%s: Warning, there seems to be no saddle point\n',called_from_string);
		%return;
		end
	end
	mr = q(1,1);
