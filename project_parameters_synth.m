function trap = project_parameters_synth
...
platform = 'matlab';
projectName = 'synth_trap_pt';                                             %%%% used in  import_data
...
simulationDirectory = '/home/nikos/qlab/trap-simulations/synthetic_trap/'; %%%% used in  import_data. 
...
NUM_ELECTRODES  = 14;                                                      %%%% used in  import_data, get_trapping_field, expand_field, dc_potential, post_process_trap
...
electrodeMapping = ...                                                     %%%% used in  import_data, expand_field
		    ...
		    11 11; 12 12; 13 13; 14 14];                                   % the convention is physical electrode -> functional electrode
                                                                           % The last entry (NUM_ELECTRODES) is the RF electrode
                                                                           % Entry NUM_ELECTRODES-1 (i.e. before the RF) is the center electrode
...
manualElectrodes = ...                                                     % used in  import_data, expand_field, dc_potential
                   [0 0 0 0 0 ...                                          % manualElectrodes determines the electrodes which are under manual voltage control. It has NUM_ELECTRODES elements
                    ...
                    0 0 0 0 ];                                             % via multipole knobs. 
                                                                           % All entries != 0 are under manual control, and entries = 0 are under multipole control 
                                                                           % If an electrode is grounded, set it under manual control, and do not set a manual voltage for it in set_voltages 
...
manualVoltages = ...                                                       % manualVoltages detrmines the voltages on the electrodes which are under manual control.
                 [0 0 0 0 0 ...                                            % It is a vector with NUM_ELECTRODES elements. See examples below. 
                  ...
                  0 0 0 0];                                                % are not confusing the code into thinking that grounded electrodes are under multipole control] 
                                                                           %
                                                                           %
                                                                           %
                                                                           % Examples: 
...
axesPermutation = [1 3 2];                                                 %%%% used in  import_data 
...
nMatTot = 1;                                                               %%%% used in  import_data, get_trapping_field
...
zMin = -10*scale;                                                          %%%% used in  import_data, get_trapping_field
zMax = 10*scale;                                                           %%%% used in  import_data, get_trapping_field
...
trappingPosition = 0*scale;                                                %%%% used in get_trapping_field, expand_field, post_process_trap
...
driveAmplitude = 500;                                                      %%%% used in post_process_trap
driveFrequency = 20e6;                                                     %%%% used in post_process_trap
...
trap.Configuration.expansionOrder = 3;                                 % this is the order to wich a spherical harmonic expansion is carried out
